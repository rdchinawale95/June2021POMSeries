...
public class AddDeleteStockInPortfolio {
	...
	public void waitForPageLoad() {
		...
		int i = 0;
		while (i != 180) {
			String pageState = (String) js.executeScript("return document.readyState;");//js query for checking the state
		...
		i = 0;
		while (i != 180) {
			Boolean jsState = (Boolean) js.executeScript("return window.jQuery != undefined && jQuery.active == 0;");//check if the
			if (jsState) {
				break;
			} else {
				waitLoad(1);
